...
export function BookingPage() {
  ...
  const [endDate, setEndDate] = useState(
    initialDateFrom ? validFutureDate(searchParams.get('dateTo')) : '',
  );
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const { res, data } = await apiFetch<BookingConfirmation & ApiErrorBody>(
        `/api/v1/listings/${id}/bookings`,
        {
          method: 'POST',
  ...
  if (confirmed) {
    return (
      <div className="max-w-lg mx-auto px-6 py-16 text-center">
        ...
        <Link
          to="/"
          className="inline-block bg-rose-500 hover:bg-rose-600 text-white font-semibold py-3 px-6 rounded-xl transition-colors"
        >
          Back to listings
        </Link>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <Link
        to={`/listings/${id}${searchParams.toString() ? '?' + searchParams.toString() : ''}`}
        ...
        {nights > 0 && (
          <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-2">
            <div className="flex justify-between text-gray-600">
              <span>
                ${listing.price_per_day} × {nights} night{nights !== 1 ? 's' : ''}
              </span>
              <span>${total}</span>
            </div>
