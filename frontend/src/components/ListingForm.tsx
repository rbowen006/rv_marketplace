...
const GENERATE_REQUIRED_FIELDS: (keyof ListingFormFields)[] = [
  'rv_type',
  'town',
  'state',
  'max_guests',
...
export function ListingForm({
  ...
  const [existingImages, setExistingImages] = useState<ListingAttachment[]>(
  ...
  async function handleGenerateDescription() {
    ...
    try {
      const { res, data } = await apiFetch<GenerateDescriptionResponse>(
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-6">
      ...
      <div>
        <label htmlFor="rv_type" className="block text-sm font-medium text-gray-700 mb-1">
          RV Type
        </label>
        <select
          id="rv_type"
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          name="pet_friendly"
          type="checkbox"
          checked={fields.pet_friendly}
          onChange={handleField}
          className="rounded border-gray-300 text-rose-500 focus:ring-rose-400"
        />
        <label htmlFor="pet_friendly" className="text-sm font-medium text-gray-700">
          Pet friendly
        </label>
      </div>
      ...
      <div>
        <label htmlFor="photos" className="block text-sm font-medium text-gray-700 mb-1">
          Photos{' '}
          <span className="text-gray-400 font-normal">(first photo will be the cover image)</span>
        </label>
          ...
          <div className="mb-3 flex flex-wrap gap-3">
            {existingImages.map((img, i) => (
              <div key={img.id} className="relative">
                {i === 0 && (
                  <span className="absolute top-1 left-1 bg-rose-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded z-10">
                    Cover
                  </span>
                )}
                <img
