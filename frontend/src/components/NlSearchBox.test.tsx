...
describe('NlSearchBox', () => {
  ...
  it('fetches immediately when the URL already carries a ?q= query', async () => {
    ...
    renderBox(['/?q=beach']);
    expect(
      await screen.findByRole('heading', { level: 3, name: 'Deep-linked Van' }),
    ).toBeInTheDocument();
  });
  ...
  it('shows an error message when the search request fails', async () => {
    request.mockResolvedValue({
      ...
      data: { message: 'Ollama is unavailable' },
    });
    renderBox();
    search('beach trip');

    expect(
      await screen.findByText(/something went wrong|unavailable|try again/i),
    ).toBeInTheDocument();
  });
  ...
  it('retries when the same query is resubmitted after a failure', async () => {
    ...
    request.mockResolvedValueOnce({
      res: { ok: true },
      data: [
        { id: 1, title: 'Recovered Van', town: 'A', state: 'X', max_guests: 2, price_per_day: 50 },
      ],
    });
    fireEvent.click(screen.getByRole('button', { name: /^search$/i }));

    expect(
      await screen.findByRole('heading', { level: 3, name: 'Recovered Van' }),
    ).toBeInTheDocument();
  ...
  it('clears the query and results when Clear is clicked', async () => {
    ...
    expect(
      screen.queryByRole('heading', { level: 3, name: 'Beachside Caravan' }),
  ...
  it('ignores a response for a query that was cleared before it resolved', async () => {
    ...
    resolve!({
      res: { ok: true },
      data: [{ id: 9, title: 'Late Van', town: 'B', state: 'Y', max_guests: 3, price_per_day: 60 }],
    });
    await waitFor(() => {});
